...
ppa_dsg <- read.csv("/scratch/scwalls/T502_RNAseq/scripts/daphnia_top_tags_AB.csv", header = F, stringsAsFactors=FALSE,sep="\t",skip=1)
...
printGraph(ppaGOdata, resultKS, firstSigNodes = 5, useInfo = "all", pdfSW = TRUE,fn.prefix="DSG_CC_AB")
write.table(allRes,file="DSG_GO_fisher_CC_AB.txt",col.names=TRUE,row.names=FALSE,quote=TRUE,sep="\t")
