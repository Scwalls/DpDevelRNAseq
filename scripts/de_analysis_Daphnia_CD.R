...
lrt <- glmLRT(fit, contrast=c(0,0,-1,1,0,0))
...
save(dge, file="daphniaDGE_CD.RData") #saving the updated dge object to our working directory
...
write.csv(daphnia_top_tags[[1]], file="daphnia_top_tags_CD.csv", row.names=FALSE) #writes a csv file to your working directory
save(daphnia_top_tags, file= "daphnia_top_tags_CD.RData") #saves the daphnia_top_tags file as a p-value
...
de_data <- dge$counts
de_data.2 <- de_data[,-13]
de_data.2 <- de_data.2[,-13]
colnames(de_data.2) <- c("A1", "A2","A3",
                       "A4", "B1","B2", "B3",
                       "B4", "C1", "C2", "D1", "D2",
                       "D5", "E1", "E2", "E3", "F1" )
head(de_data.2)
...
de_data.2 <- cbind(de_data.2, top_tags[[1]])
head(de_data.2)

diff.genes = rownames(de_data.2[de_data.2$FDR<0.01, ])
head(diff.genes)
length(diff.genes)
